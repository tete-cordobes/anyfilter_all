...
Version 0.1.5 recognizes the `ytp-ad-badge--clean-player` badge observed in the user's third live export. Previously, that layout needed a ready skip button to satisfy the local ad-UI gate; the badge now allows detection and a Jev decision before skipping becomes available. The player must still show an active ad, the badge must be visible and both Jev scores must pass the threshold. The live export confirms a Jev evaluation and two automatic skip attempts, but the user also pressed Skip manually: the return to main content does not validate automatic skipping. It contains no 16× attempt. The badge change is verified on a reconstructed fixture; live automatic skipping and no-button acceleration remain to be checked. See [RESULTS.md](RESULTS.md) for the evidence and remaining limits.
