...
Version 0.1.5 recognizes the `ytp-ad-badge--clean-player` badge observed in the user's third live export. Previously, that layout needed a ready skip button to satisfy the local ad-UI gate; the badge now allows detection and a Jev decision before skipping becomes available. The player must still show an active ad, the badge must be visible and both Jev scores must pass the threshold. The live export confirms a Jev evaluation, two skip attempts and a return to main content in one episode; it contains no 16× attempt. The change is verified on a reconstructed fixture, with live no-button acceleration still to be checked. See [RESULTS.md](RESULTS.md) for the evidence and remaining limits.
